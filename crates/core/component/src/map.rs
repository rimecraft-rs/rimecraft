    ...
    Cx: ProvideIdTy,
{
        ...
        T: Send + Sync + 'a,
    {
        ...
        match &mut self.0 {
            ...
            MapInner::Patched { base, changes, .. } => {
                ...
                if old.is_some_and(|old| (ty.f.util.eq)(&val, old)) {
                    ...
                    return old
                        .and_then(|old| unsafe { <dyn Any>::downcast_ref(old) })
                        .map(Maybe::Borrowed);
                }
                .flatten()
            }
        ...
        .and_then(|obj| unsafe { downcast_boxed(obj).ok() })
            ...
            MapInner::Patched { base, changes, .. } => {
                ...
                match (old, now) {
                    (Some((k, v)), None) => {
                        ...
                        unsafe { <dyn Any>::downcast_ref(v).map(Maybe::Borrowed) }
                    }
                    (Some(_), Some(now)) => now
                        .take()
                        .and_then(|obj| unsafe { downcast_boxed(obj).ok() })
